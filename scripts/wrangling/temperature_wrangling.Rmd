...
hob_filelist <- list.files(here("july2024_updates/data/HOBO_temperature_loggers/CSVs")) 
...
name <- read_csv(here(paste("july2024_updates/data/HOBO_temperature_loggers/CSVs/", hob_filelist[1], sep = "")), skip=1) 
...
for (file in hob_filelist[-1]) { # skip the first file because that's what we did above
  ...
  name <- read_csv(here(paste("july2024_updates/data/HOBO_temperature_loggers/CSVs/", file, sep = "")), skip=1) # read in each csv
...
hob_metadat <- read_csv(here("july2024_updates/data/HOBO_temperature_loggers/HOBO_metadata.csv"))
...
```{r}
write_csv(x = temp_time_avgs, file = here("july2024_updates/generated_data/hobo_timeseries.csv"))
```
